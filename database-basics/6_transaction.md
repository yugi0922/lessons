...
6.4.1 同時実行で発生する異常現象
独立性（Isolation）を完全に保証するとパフォーマンスが低下するため、多くのデータベースでは、その保証レベルを意図的に緩和することが許されています。

この保証レベルを「分離レベル（Isolation Level）」と呼びます。分離レベルが低いほど、パフォーマンスは向上しますが、以下のようなデータ不整合を引き起こす「異常現象」が発生する可能性があります 。   
