...
## まとめ リレーショナルデータベースの構造

![sample](./images/3_06_RDB_architecture_basic.svg)

5つの主要要素とその関係を階層的に表現：
1. クライアント・サーバー（左側）

4つのクライアント（Webアプリ、BIツール、バッチ、管理ツール）

SQL文インターフェースによる統一的なデータアクセス
各クライアントからサーバーへの接続線

2. プロセス構造（接続管理）（最上層）

接続プロセス：1接続=1プロセスの管理
バックグラウンドプロセス：Checkpoint、Log Writer、Vacuum
メモリ/I/O管理：Buffer Manager、I/O Manager、Lock Manager

3. メモリ構造（第2層）

バッファプール：データページキャッシュ、LRU管理
ログバッファ：WAL一時保持、トランザクション管理
ソート領域：ORDER BY処理、一時作業領域
共有リソース：ロックテーブル、統計情報、カタログ

4. ストレージエンジン（第3層）

物理構造管理：B+ツリー、ヒープ、ページ管理
トランザクション処理：ACID特性、ロック、コミット、ロールバック
データ永続化：ディスク書き込み、同期/非同期I/O
クラッシュリカバリ：WAL機能、Redo/Undo、自動復旧

5. ファイルシステム（物理ストレージ）（最下層）

データファイル：テーブル行データ、ページ単位管理
ログファイル：更新履歴、クラッシュ復旧用
インデックスファイル：検索高速化、B+ツリー構造
設定・一時ファイル：DB設定、ソート用一時領域

このようにに複数の層が連携してACID特性を保証し、高性能なデータ処理を実現している