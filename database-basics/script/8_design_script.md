皆さん、こんにちは。  
本日は「リレーショナルデータベース（RDB）のスケーラビリティ・可用性・セキュリティ」について、実運用の観点から解説していきます。
...
どれか1つではなく「3つのバランス」を取っていくための視点を持つようにしてください
...
スケーラビリティやシャーディングなどの施策を検討する前に、まずは「1台のDBでどこまで性能を出せるか」を最大化することが重要です。  
...
2. **クエリの高速化**  
   `WHERE created_at >= '2024-07-01'` のような条件があると、必要なパーティションだけをスキャンすることができます。
...
具体的には、データベースを複数のサーバーやインスタンスに分散して保存する、という形です
...
- 復旧手順書は整備されているか？  
- PITR（障害発生直前までデータを復旧）やスナップショット復旧の演習を最近行ったか？
...
さらに、監視と演習です。  
監視はインフラ・DB・アプリの各層で行い、**slowSQLの監視やレプリケーション遅延のチェック**を日常的に回すことが欠かせません。  
...
- 次に、**slowSQLを監視する仕組み**が現状あるかを確認してください。もしなければ導入を検討しましょう。  
