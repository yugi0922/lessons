...
- パブリッククライアントの場合は、PKCE (Proof Key for Code Exchange) という仕組みを使ってセキュリティを高めることが推奨されます。
...
役割
- リソースオーナーの認証
    - リソースオーナーが本人であることを確認します（例: ID/パスワード入力、生体認証など）。
- リソースオーナーの同意取得
    - クライアントがどの情報にアクセスしようとしているか（スコープ）をリソースオーナーに提示し、許可を得ます。
- アクセストークンの発行
    - 認証と同意が成功したら、クライアントに対してアクセストークンを発行します。場合によってはリフレッシュトークンも発行します。
...
- client_id
    - クライアントの識別子。
- redirect_uri
    - 認可コードを受け取るクライアント側のURI。事前に登録が必要です。
- scope
    - 要求する権限の範囲（例: profile email）。
- state
    - CSRF（クロスサイトリクエストフォージェリ）攻撃を防ぐためのランダムな文字列。クライアントが生成し、後に検証します。
...
- PKCE (Proof Key for Code Exchange) の利用
    - パブリッククライアント（モバイルアプリやSPAなど、クライアントシークレットを安全に保持できないクライアント）の場合、認可コードの横取り攻撃を防ぐためにPKCEの利用が強く推奨（OAuth 2.1では必須）されます。
...
### なぜこのフローが重要か？
- マシン対マシン (M2M) 通信の標準化
    - クライアントアプリケーション（例: バックエンドサーバー、バッチ処理システム）が、ユーザーの介在なしに、自身の権限でリソースサーバーへアクセスするための明確で安全な方法を提供します。これは、マイクロサービスアーキテクチャや自動化されたプロセスにおいて不可欠です。
- 自律的なクライアント操作
    - クライアントが「自分自身として」動作し、自身が管理するリソースにアクセスしたり、自身に許可された操作（例: APIへの定期的なデータ送信、サービス設定の更新など）を実行する場合に最適です。
- シンプルさと効率性
    - ユーザーの認証や同意のステップが存在しないため、フローが非常にシンプルです。プログラムによる自動化に適しており、効率的な連携を実現します。
- 責任範囲の明確化
    - このフローで発行されるアクセストークンは、特定のユーザーではなくクライアント自身に関連付けられるため、APIアクセスにおける責任の所在が明確になります。
...
### なぜこのフローが重要か？
- 入力が困難なデバイスの救世主
    - スマートTV、ゲーム機、IoTデバイス、CLI（コマンドラインインターフェース）ツールなど、キーボード入力が煩雑だったり、ブラウザを搭載していなかったりするデバイスでも、OAuth 2.0による安全な認可を実現できる点が最大の重要性です。これらのデバイスでユーザーのパスワードを直接入力させるのはセキュリティリスクが高く、ユーザー体験も損ないます。
- ユーザー体験の向上
    - ユーザーは、使い慣れたスマートフォンやPCといったセカンダリデバイスのブラウザを使って、簡単かつ安全に認証・認可操作を行えます。デバイス本体での面倒な操作が不要になります。
-  OAuth 2.0の適用範囲拡大
    - このフローの存在により、OAuth 2.0を利用できるアプリケーションやサービスの幅が大きく広がりました。多様なデバイスがセキュアに連携するための道筋を提供しています。
-  セキュリティの維持
    - ユーザーの認証情報を入力制限のあるデバイスに直接渡すことなく、アクセストークンを取得できるため、セキュリティレベルを維持したまま連携が可能です。
...
## 2.5. トークン
OAuth 2.0 のフローにおいて、クライアントがリソースへアクセスする際や、ユーザーの認証情報を扱う際に「トークン」と呼ばれる資格情報がやり取りされます。トークンは、認可サーバーによって発行され、それぞれ異なる目的と特性を持っています。

![thumbnail](./images/OAuth2.0zu5.png)

## 2.5.1. アクセストークン (Access Token)
### 役割
クライアントが保護されたリソース（例: API）へアクセスする際に使用する「鍵」の役割を果たします。リソースサーバーは、このアクセストークンを検証することで、クライアントにリソースへのアクセスを許可するかどうかを判断します。
...
### 内容
- クライアントにとっては通常「不透明 (Opaque)」な文字列として扱われます。つまり、クライアントはアクセストークンの中身を解釈する必要はありません（できません）。
- ただし、JWT (後述) のように構造化された形式で、トークン自体が情報（例: 有効期限、スコープ、ユーザーIDなど）を含むこともあります。

### 有効期間
セキュリティ上の理由から、一般的に短命（数分～数時間程度）に設定されます。有効期限が切れた場合は、リフレッシュトークン（後述）を使って新しいアクセストークンを取得します。

### 送信方法
通常、HTTPリクエストの Authorization ヘッダーに Bearer <アクセストークン> の形式で含めてリソースサーバーに送信されます。

### ポイント: 
**リソースアクセスの「通行許可証」や「鍵」**と考えると分かりやすいでしょう。

## 2.5.2. リフレッシュトークン (Refresh Token)

### 役割
アクセストークンの有効期限が切れた際に、ユーザーに再認証を求めることなく、新しいアクセストークンを取得するために使用されるトークンです。
...
### 内容
クライアントにとっては不透明な文字列です。

### 有効期間
アクセストークンよりも長命（数日～数ヶ月、あるいは無期限の場合も）に設定されることが一般的です。

### セキュリティ上の注意
- リフレッシュトークンはアクセストークンを再発行できる強力な権限を持つため、非常に厳重に保管・管理される必要があります。特にConfidential Client（サーバーサイドアプリケーション）で安全に保管します。
- 漏洩時のリスクを軽減するため、リフレッシュトークンローテーション（一度使用したら新しいリフレッシュトークンも発行する）や、発行しない、あるいは短命にするなどの戦略も取られます。
- Public Client（SPAやモバイルアプリなど、安全な保管が難しいクライアント）には、原則として発行しないか、発行する場合でもPKCEで保護されたアクセストークンと紐づけるなどの追加のセキュリティ対策が考慮されます。

### ポイント
アクセストークンの「再発行券」。これがあれば、ユーザーの手を煩わせずにアクセスを継続できます。

## 2.5.3. IDトークン (ID Token) ※OIDCの範囲
### 役割
OpenID Connect (OIDC) という、OAuth 2.0 を拡張した認証プロトコルで使用されます。クライアントがユーザーの認証イベントに関する情報（ユーザーが誰であるか、いつどのように認証されたかなど）を取得するためのトークンです。

### 発行者
認可サーバー (OIDCではOpenID Providerとも呼ばれる)

### 提示先
クライアントアプリケーション

### 内容
必ず JWT (JSON Web Token) 形式で発行されます。ユーザーの識別子 (sub)、発行者 (iss)、クライアントの識別子 (aud)、有効期限 (exp)、発行日時 (iat) などの標準的なクレーム（情報）に加え、ユーザー名やメールアドレスなどのプロフィール情報を含むことができます。

### 目的
認証。リソースへのアクセス権限（認可）を与えるのはアクセストークンの役割であり、IDトークンは主に「ユーザーが誰であるか」をクライアントに伝えるために使われます。

### ポイント
ユーザーの「デジタル身分証明書」。クライアントはIDトークンを検証することで、ユーザーが確かに認証されたことを確認できます。

## 2.5.4. トークンの種類・表現方法
上記で説明したトークンは、その表現方法や利用時の特性によって、以下のようなタイプに分類されることがあります。

### a. Bearer Token (ベアラートークン)
- 特徴
    - 「このトークンを持っている人（Bearer）なら誰でも使える」という意味を持つトークンです。最も一般的なアクセストークンの形式です。
- 送信方法
    - HTTPの Authorization ヘッダーに Authorization: Bearer <token> のように指定します。
- メリット
    - 実装がシンプルです。
- デメリット
    - トークン自体に所有者の情報が含まれないため、盗まれた場合、第三者にそのまま悪用されるリスクがあります。そのため、HTTPSによる通信の暗号化が必須であり、トークンの保管にも細心の注意が必要です。

### b. JWT (JSON Web Token) (ジェイソンウェブトークン)
- 構造
    - クレーム（情報）をJSONオブジェクトとして表現し、電子署名や暗号化を施すことができる、コンパクトでURLセーフなトークン仕様です。ヘッダー・ペイロード・署名の3つの部分から構成され、それぞれBase64URLエンコードされてピリオド (.) で連結されます。 (例: xxxxx.yyyyy.zzzzz)
- 特徴:
    - 自己完結型: トークン自体に必要な情報（クレーム）を含めることができます。
    - 検証可能: 電子署名により、トークンが改ざんされていないこと、そして信頼できる発行者によって発行されたことを検証できます。
    - ステートレス: リソースサーバーは、発行者の公開鍵などを使ってJWTを検証できれば、必ずしも認可サーバーに問い合わせることなくトークンの有効性を確認できる場合があります（トークンイントロスペクションという問い合わせる方法も一般的です）。
- 用途
    - IDトークンは必ずJWT形式です。アクセストークンもJWT形式で発行されることがあります。
- 注意点
    - ペイロード部分はBase64URLエンコードされているだけであり、暗号化されているわけではありません（誰でもデコードして内容を読めます）。機密情報を含める場合はJWE (JSON Web Encryption) を使って暗号化する必要があります。
    - 署名は改ざん検知のためであり、ペイロードの秘匿性を保証するものではありません。

### c. DPoP (Demonstration of Proof-of-Possession) (ディーポップ)
- 目的
    - Bearer Token の「誰でも使える」という弱点を克服し、**アクセストークンを特定のクライアントに紐付ける（Sender-Constrained Token）**ための仕組みです。トークンが盗まれても、攻撃者が正当なクライアントになりすましてトークンを再利用することを困難にします。
- 仕組み (概要)
1.クライアントは公開鍵と秘密鍵のペアを生成します。
2.アクセストークンを要求する際や、アクセストークンを使ってリソースサーバーにアクセスする際に、「DPoPプルーフJWT」と呼ばれる特別なJWTを生成します。このJWTには、HTTPメソッド、リクエストURI、およびクライアントが持つ公開鍵の情報などが含まれ、クライアントの秘密鍵で署名されます。
3.認可サーバーやリソースサーバーは、このDPoPプルーフJWTとアクセストークンを検証し、正当なクライアントからのリクエストであることを確認します。
- メリット
    -  Bearer Token に比べて、トークン盗難・再利用に対するセキュリティが大幅に向上します。
- 関連
    - OAuth 2.1 では、Bearer Token のセキュリティを向上させるためのより強固なメカニズムとして、DPoPのようなProof-of-Possession技術の採用が推奨される方向です。

#### これらのトークンの種類と特性を理解することは、OAuth 2.0 を安全かつ効果的に利用するための基礎となります。アプリケーションの要件やセキュリティレベルに応じて、適切なトークンの取り扱いを検討することが重要です。
...
- HTTPS通信の徹底
    - クライアント、認可サーバー、リソースサーバー間の全ての通信（特にトークンの送受信）は必ずTLS (HTTPS) で暗号化します。これにより、通信経路上での盗聴を防ぎます。
- アクセストークンの有効期間を短く
    - アクセストークンの有効期間は必要最小限に短く設定します（例: 数分～1時間程度）。万が一漏洩しても、利用できる時間が限られるため被害を抑えられます。有効期間が短いトークンは、リフレッシュトークンを使って再発行します。
- Bearer Token の取り扱い注意
    - デフォルトで使われる Bearer Token は、そのトークンを持っているだけで誰でもリソースにアクセスできてしまうため、特に送信時や保存時の取り扱いに注意が必要です。
